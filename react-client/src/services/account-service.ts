...
export class AccountService {
  static async login (loginInfo: ILoginInfo): Promise<IJwtResponse | null> {
    return await axios.post<IJwtResponse>('https://icd0009.azurewebsites.net/api/v1/Account/Login',
  ...
  static async register (registerInfo: IRegisterInfo): Promise<IJwtResponse | null> {
    return await axios
      .post<IJwtResponse>('https://icd0009.azurewebsites.net/api/v1/Account/Register', registerInfo,
        { headers: { 'Content-type': 'application/json' } }
      ).then(async (response: AxiosResponse<IJwtResponse>) => {
        if (response.status > 199 && response.status < 300) {
          return response.data
